...
export function CopyButton({ value, targetRef, className = '' }: CopyButtonProps) {
  ...
  return (
    <button
      type="button"
      onClick={handleCopy}
      className={`inline-flex items-center gap-2 rounded-full border-2 border-tinta/15 px-4 py-2 font-mono text-sm uppercase tracking-wide text-tinta hover:border-rosa hover:bg-rosa-claro/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rosa focus-visible:ring-offset-2 ${className}`}
    >
